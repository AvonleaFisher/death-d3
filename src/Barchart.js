...
const drawChart = (data, bin) => {
	...
	allBars.enter()
		...
		.on("mouseover", (d, i)=>{
			...
			for (var i = 0; i < d.length; i++){
			 	allData += textFormat(d[i].DeceasedFirstName) + " " + textFormat(d[i].DeceasedLastName) + " " +  "died on" + " " + d[i].DateOfDeath + " " + "at" + " " + textFormat(d[i].prisonOfDeath) + ", " + d[i].Miles + " " + "miles from home" + "<br/>";
			}
